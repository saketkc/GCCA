...
class BridgedCCA(GCCA):
    ...
    def fit(self, x0_pair0, x1_pair0, x1_pair1, x2_pair1):
        ...
        p0_d_list = [0] + [
            sum([len(x.T) for x in p0_list][:i + 1]) for i in range(p0_num)
        ]
        p1_d_list = [0] + [
            sum([len(x.T) for x in p1_list][:i + 1]) for i in range(p1_num)
        ]
        ...
        x1_var += self.reg_param * np.average(np.diag(x1_var)) * np.eye(
            x1_var.shape[0])
        ...
        right = np.vstack([
            np.hstack([c00, np.zeros_like(c01),
                       np.zeros_like(c02)]),
            np.hstack([np.zeros_like(c01.T), c11,
                       np.zeros_like(c12)]),
            np.hstack([np.zeros_like(c02.T),
                       np.zeros_like(c12.T), c22])
        ])
